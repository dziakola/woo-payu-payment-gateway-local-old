(function ($) {
    ...
    $('form#order_review').on('submit', function (e) {
        ...
        } else if (paymentMethod === 'payublik') {
          validateResult = validate_blik_widget();
        }
    ...
    $form.on("checkout_place_order_payublik", function () {
        return validate_blik_widget();
    });

    function validate_form(condition, container, errorField) {
      if (condition) {
        $("html, body").animate(
          {
            scrollTop: $('.payu-' + container).offset().top,
          },
          300
        );
        $(errorField + '-error').slideDown(250);

        return false;
      } else {
        $(errorField + '-error').slideUp(250);
    ...
    function validate_payu_list_banks() {
      var condition = !($('.payu-list-banks').find('.payu-active .active').length);
      return validate_form(condition, 'list-banks', '.pbl');
    }

    function validate_blik_widget() {
      var blikInputCode = $("#BLIK_AUTHORIZATION_CODE").val();
      var regex = /^[0-9]{6}$/;
      var condition = !regex.test(blikInputCode);
      return validate_form(condition, 'blik-container', '.blik');
    }
})(jQuery);
