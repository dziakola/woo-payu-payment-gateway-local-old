...
define('PAYU_PLUGIN_VERSION', '2.0.33');
...
function payu_plugin_on_activate()
{
    if (!get_option('woocommerce_payu_settings') && !get_option('_payu_plugin_version')) {
        ...
        //add BLIK code checkbox to admin options
        add_option(
            'woocommerce_payublik_settings',
            [
                'enabled' => 'no',
                'BLIK_T6_enabled' => 'yes'
            ]
...
function view_order($order_id)
{
    ...
    if (in_array($order->get_status(), ['on-hold', 'pending', 'failed'])
        && array_key_exists($order->get_payment_method(), $payu_gateways)
