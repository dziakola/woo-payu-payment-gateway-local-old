...
abstract class WC_PayUGateways extends WC_Payment_Gateway
{
    ...
    function payu_init_form_fields($custom_order = false)
    {
        ...
        $this->form_fields = array_merge(
            ...
            $this->get_additional_blik_fields(),
    ...
    /**
     * return array with additional admin BLIK options
     * @return array
     */
    ...
    function process_payment($order_id)
    {
        ...
        try {
            ...
            $code = $e->getOriginalResponse()->getResponse()->status->codeLiteral;
            switch ($code) {
                case 'AUTH_CODE_EXPIRED':
                    $message = __('The authorization code has expired.', 'woo-payu-payment-gateway');
                    break;
                case 'AUTH_CODE_USED':
                    $message = __('The authorization code has been used already.', 'woo-payu-payment-gateway');
                    break;
                case 'AUTH_CODE_EXCEEDED':
                    $message = __('The limit of the authorization code has been exceeded.', 'woo-payu-payment-gateway');
                    break;
                case 'AUTH_CODE_CANCEL':
                    $message = __('The authorization code has been canceled.', 'woo-payu-payment-gateway');
                    break;
                case 'AUTH_CODE_INVALID':
                    $message = __('Incorrect authorization code.', 'woo-payu-payment-gateway');
                    break;
                case 'INVALID_BLIK_CODE':
                    $message = __('Syntactically incorrect BLIK authorization code. BLIK authorization code should consist of 6 digits.', 'woo-payu-payment-gateway');
                    break;

                default:
                    $message = __('Blik payment issue, contact shop to ask about error', 'woo-payu-payment-gateway') . '(' .  $e->getOriginalResponse()->getResponse()->status->codeLiteral . ')';
            }
    ...
    private function getProducts($order)
    {
        ...
        if (!empty($order->get_shipping_methods())) {
            $products[] = [
                'name' => substr('Shipment' . ' [' . $order->get_shipping_method() . ']', 0, 255),
    ...
    private function getThreeDsAuthentication($order, $orderData)
    {
        if (!isset($orderData['payMethods'])
            ...
            $name = mb_substr($order->get_formatted_billing_full_name(), 0, 10);
            ...
            if (!empty($name) || $isBillingAddress) {
                ...
                if (!empty($name)) {
                    $threeDsAuthentication['cardholder']['name'] = mb_substr($order->get_formatted_billing_full_name(), 0, 10);
                }
